...
#run this on machine
observed_path = os.path.join(path, "../../../../TestComponents/Observed/observed.csv")
...
Alltests =[]
for t in tests[:]:  
    testframe = pd.read_csv(out_path+"\\"+t,index_col=0,dayfirst=True,date_format='%d/%m/%Y %H:%M:%S %p')  
